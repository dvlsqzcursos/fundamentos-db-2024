...
SELECT COUNT(*)
FROM Productos
WHERE CAST(Saldo AS UNSIGNED) > 0;

SELECT *
FROM Productos
WHERE CAST(Saldo AS UNSIGNED) < 5;
...
SELECT Cilentes_idCilentes, COUNT(*) AS cantidad_compras
FROM Ventas
GROUP BY Cilentes_idCilentes
ORDER BY cantidad_compras DESC
LIMIT 1;

SELECT Proveedores_idProveedores, COUNT(*) AS cantidad_adquisiciones
FROM Compras
GROUP BY Proveedores_idProveedores
ORDER BY cantidad_adquisiciones DESC
LIMIT 1;

SELECT SUM(Precio_venta * CAST(Saldo AS UNSIGNED)) AS valor_total
FROM Productos
WHERE CAST(Saldo AS UNSIGNED) > 0;
